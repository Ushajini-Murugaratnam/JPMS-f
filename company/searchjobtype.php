                ...
                <div class="card shadow mb-4">
                    <!-- User Details Table -->
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                            <h6 class="m-0 font-weight-bold text-primary">Type with jobs </h6>
                         </div>
                                ...
                                <div class="form-group">
                                        <select name="type_id" id="" class="form-control form-control-user">
                                            <?php
                                                ...
                                                $find="SELECT DISTINCT type_id ,job_type FROM `tbl_job_type`";
                                                $result=mysqli_query($con,$find);
                                                while($row=mysqli_fetch_array($result))
                                            {
                                                echo "<option value='".$row['type_id']."'> ".$row['type_id']."  ".$row['job_type']." </option>";
                                            }
                                            ?>
                                        </select>
                                </div>
                                                ...
                                                on tbl_job.type_id=tbl_job_type.type_id 
                                                    LEFT join tbl_company 
                                                        on tbl_job.company_id=tbl_company.company_id
                                                            where tbl_job.type_id=$type_id and tbl_company.company_id = $loggedin_id and tbl_job.apStatus=1";
